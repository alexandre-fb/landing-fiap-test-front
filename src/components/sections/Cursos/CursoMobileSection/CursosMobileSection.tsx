        ...
        <ul className={styles.accordionList}>
          {courses.map((course) => (
            <li key={course.value} className={styles.accordionItem} >
              <button type="button" className={styles.accordionButton} onClick={() => handleAccordion(course.value as courseValue)}>
                <h3>{course.label}</h3>
                <span className={`${styles.accordionIcon} ${openTab === course.value ? styles.active : ""}`}>
              ...
              {openTab === course.value && (
                <div className={styles.accordionPanel}>
                  <CourseContent
                    ...
                    courseList={courseListByCategory[course.value as courseValue]}
                  />
                </div>
              )}
            </li>
